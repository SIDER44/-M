...
function togglePlay() {
  if (!State.currentTrack) return showToast("No track selected");
...
function toggleMute() {
  ...
  const volIcon = document.getElementById("volIcon");
  volIcon.className = audio.muted ? "fa fa-volume-mute" : "fa fa-volume-up";
}
...
async function playTrack(track, addToQueue = true) {
  ...
  try {
    ...
    showToast("Press play to start", "error");
  }
...
async function renderHome() {
  ...
  content.innerHTML = `
    <div class="hero-banner">
      <div class="hero-text">
        ...
        <p class="hero-subtitle">Millions of songs, playlists & podcasts. Free.</p>
        <div class="hero-actions">
          ...
          <button class="btn-outline" onclick="navigate('genres')">
            <i class="fa fa-music"></i> Browse Genres
          </button>
        </div>
      </div>
  ...
  try {
    ...
    const trendingEl = document.getElementById("homeTrending");
    if (!trendingEl) return;
    if (tracks.length === 0) {
      trendingEl.innerHTML = `<div class="empty-state"><div class="empty-icon">😔</div><p>No trending data available</p></div>`;
      return;
    }
    trendingEl.innerHTML = renderCardsGrid(tracks.slice(0, 8));
  } catch (e) {
    const el = document.getElementById("homeTrending");
    if (el) el.innerHTML = `<div class="empty-state"><div class="empty-icon">⚠️</div><p>Failed to load trending</p></div>`;
  }
}

function renderHomeArtists() {
  const artists = ["Drake", "Taylor Swift", "Burna Boy", "Bad Bunny", "Wizkid", "Beyoncé"];
  const emojis = { "Drake": "🎤", "Taylor Swift": "🎸", "Burna Boy": "🌍", "Bad Bunny": "🐰", "Wizkid": "⭐", "Beyoncé": "👑" };
  const artistsEl = document.getElementById("homeArtists");
  if (!artistsEl) return;

  artistsEl.innerHTML = `<div style="display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;">
    ${artists.map(a => `
      <div onclick="searchArtist('${a}')"
        style="flex-shrink:0;text-align:center;cursor:pointer;padding:16px 12px;background:var(--bg-card);border-radius:var(--radius);width:120px;transition:all 0.2s;"
        onmouseover="this.style.background='var(--bg-card2)'" onmouseout="this.style.background='var(--bg-card)'">
        <div style="width:70px;height:70px;border-radius:50%;background:linear-gradient(135deg,var(--primary),#ff4444);margin:0 auto 10px;display:flex;align-items:center;justify-content:center;font-size:1.6rem;">
          ${emojis[a]}
        </div>
        <div style="font-size:0.82rem;font-weight:600;">${a}</div>
      </div>
    `).join("")}
  </div>`;
}
...
function renderSearch() {
  ...
  content.innerHTML = `
    <div class="search-hero">
      <h2>Search ALMEER MUSIC</h2>
      <p>Find your favorite songs, artists and albums</p>
      <div class="search-big-input">
        <input id="searchBigInput" placeholder="Search songs, artists..." onkeyup="handleSearchBigKey(event)" autofocus />
...
function performSearch() {
  ...
  navigate("search");
  setTimeout(() => {
    if (document.getElementById("searchBigInput")) {
      document.getElementById("searchBigInput").value = q;
    }
    doSearch(q);
  }, 100);
}
...
function performBigSearch() {
  const q = document.getElementById("searchBigInput")?.value?.trim();
  if (q) doSearch(q);
}
...
function searchArtist(name) {
  ...
  setTimeout(() => {
    if (document.getElementById("searchBigInput")) {
      document.getElementById("searchBigInput").value = name;
    }
    doSearch(name);
  }, 100);
}
...
async function doSearch(q) {
  ...
  try {
    ...
    if (tracks.length === 0) {
      resultsEl.innerHTML = `<div class="empty-state"><div class="empty-icon">🔍</div><h3>No results found</h3><p>Try a different search term</p></div>`;
      return;
    }

    resultsEl.innerHTML = `
      <div class="section-header">
        <h2 class="section-title">Results for "${escapeHtml(q)}"</h2>
        <span class="text-muted">${tracks.length} songs</span>
      </div>
      <div class="track-list">${tracks.map((t, i) => renderTrackItem(t, i, tracks)).join("")}</div>
    `;
  } catch (e) {
    resultsEl.innerHTML = `<div class="empty-state"><div class="empty-icon">⚠️</div><p>Search failed. Try again.</p></div>`;
  }
}
...
async function renderTrending() {
  ...
  try {
    ...
    if (tracks.length === 0) {
      listEl.innerHTML = `<div class="empty-state"><div class="empty-icon">📡</div><h3>No trending data</h3><p>Check back soon</p></div>`;
      return;
    }

    listEl.innerHTML = `<div class="track-list">${tracks.map((t, i) => renderTrackItem(t, i, tracks)).join("")}</div>`;
  } catch (e) {
    const el = document.getElementById("trendingList");
    if (el) el.innerHTML = `<div class="empty-state"><div class="empty-icon">⚠️</div><p>Failed to load trending</p></di
